...
var _warning = exports.warning = function(Template,warning){
	...
	if(typeof warning === "string"){
		Template._warnings.push({warning:Template.language.warning(warning),lineNumber:lineCount});
	}
	else if(typeof warning !== "undefined"){
		i = warning.length;
		while(i--){
			Template._warnings.unshift({warning:Template.language.warning(warning),lineNumber:lineCount});
		}
	}
};
/**--Noodles.Utilities--
...
var _parseType = exports.parseType = function(Template,input){
	if(typeof _Template === "undefined") _Template = require('./template').Template;
	input = input.trim();
	if(!(Template instanceof _Template)) _warning(Template,"parseType expects its first argument to be a Noodles Template");
	if(/^\-?\d+\.?\d*$/.test(input)){
...
var _grabToEndSliceRaw = exports.grabToEndSliceRaw = function(Template, expression,tag){
	...
	while(needsEnding > 0){
		...
		else if(needsEnding !== -1){
			tag = tag || 'Tag'
			_warning(Template,tag + Template.language.warning(' was improperly ended'));
