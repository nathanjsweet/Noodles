...
Context.prototype.getObject = function(Template,key,override){
	...
	for(var i = 0, l = key.length; i < l; i++){
		keyTemp = typeof key[i].execute !== "undefined" ? key[i].execute(Template,this).toLowerCase() : key[i];
...
Context.prototype.setObject = function(Template, key,value){
	...
	for(var i = 0, l = key.length - 1; i < l; i++){
		keyTemp = typeof key[i].execute !== "undefined" ? key[0].execute(Template,this).toLowerCase() : key[i];
		if(typeof obj[keyTemp] !== "undefined"){
			obj = obj[keyTemp];
		}
		else{
