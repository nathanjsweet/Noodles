...
Context.prototype.getObject = function(Template,key,override){
	...
	var keyTemp = key[0].toLowerCase(),
		obj,keys,keyIndex;
	if(!this._metaSkip[keyTemp] && this._metaData.hasOwnProperty(keyTemp)){
		return this._metaData[keyTemp].execute(Template,this);
	}
	obj = this._others;

	for(var i = 0, l = key.length; i < l; i++){
	    keyTemp = typeof key[i].execute !== "undefined" ? key[i].execute(Template,this).toLowerCase() : key[i].toLowerCase();
	...
	return typeof obj.execute !== "undefined" ? obj.execute(Template,this) : typeof obj === "object" && !override ? ' ' : obj;
};
/**--Noodles.Context--
name:setObject
...
Context.prototype.setObject = function(Template, key,value){
	...
	for(var i = 0, l = key.length - 1; i < l; i++){
	    keyTemp = typeof key[i].execute !== "undefined" ? key[0].execute(Template,this).toLowerCase() : key[i].toLowerCase();
		if(typeof obj[keyTemp] !== "undefined"){
			obj = obj[keyTemp];
		}
		else{
...
Context.prototype.error = function(Template,error){
	...
	this.errors.push({
		error:Template.language.error(error + 'on line ' + lineNumber + '.'),
