...
@return {*}
*/
_Object.prototype.execute = function(Template,Context,callback){
	if(!this._stringOnly){
		try{
			return Context.getObject(Template,this.order,callback);
		}
		catch(e){
			console.log(e);
			return false;
...
@param {*}
*/
_Object.prototype._set = function(Template,Context,value,callback){
	if(!this.meta){
		try{
			value = typeof value.execute !== "undefined" ? value.execute(Template,Context) : value;
			Context.setObject(Template,this.order,value,callback);
		}
		catch(e){
			console.log(e);
			return false;
