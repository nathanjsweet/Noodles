...
@param{string}
*/
var _warning = exports.Utilities.templateCreationWarning = function(Template,warning){
	var lineCount = _getLineCount.call(Template),
		i;
...
_Object.prototype.execute = function(Context){
	...
	if(!this.meta || !this._stringOnly){
		var temp = Context.getObject(order[i]),
			order = this.order,
			l = this.olength,
			i = 1,
			key;
		...
		return _self._metaData[this.metaKey].execute(Context);
	}
...
@return {*}
*/
var _parseType = exports.parseType = function(_self,input){
	input = input.trim();
	
	if(/^\d+\.?\d*$/.test(input)){
		return new _Number(_self,input);
	}
	else if(/^('|")[^'"]+\1{1}$/.test(input)){
		return new _String(_self,input);
	}
	else if(reIdentifier.test(input)){
		return new _Object(_self,input);
	}
	else{
		return input;
	}
