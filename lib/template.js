...
/**--Template--
name:passNoodles
description: function that is passed the Noodles
	object on compile so that template can have
	a reference to plugins.
@param{Noodles}
*/
exports.passNoodles = function(Noodles){
	_Noodles = Noodles;
	delete exports.passNoodles;
}
/**--module--
...
/**--Noodles--
name:_grabPlugin - private method
...
var _grabPlugin = function(pluginName,callback,deep){
	var file = (deep ? './../../../noodles_plugins/' : './../plugins/') + pluginName + '/' + pluginName,
		_self = this;
	try{
		require([file],function(plugin){
			...
			if(typeof plugin.getNoodles !== "undefined"){
				plugin.getNoodles(_Noodles);
			}
			plugin = new _Noodles.Plugin(plugin);
		...
		if(deep)
			...
			return _grabPlugin.call(this,pluginName,callback,true);
	}
};
