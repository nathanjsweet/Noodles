...
var _registerMaster = function(objArg){
	...
	Globalization.language(objArg.language,function(language){
		_self.language = language;
		_self._plugins = _self.language.coreTags.slice();
		...
		if(metaData.length > 0){
			while(i--){
				...
				if(_Object.reIdentifier.test(name) && meta[2] === '='){
					if(name === _self.language.other("plugins")){
						_self._plugins = _self._plugins.concat(value.toLowerCase().split(','));
						continue;
					}
					switch(name[0]){
						case '\\':
							_self._metaData[name.slice(1)] = new _Template({rawString:value,executeOnce:true,metaKey:name.slice(1)},_self);
							break;
						case '#':
							break;
						default:
							_self._metaData[name] = new _Template({rawString:value,executeOnce:false},_self);
					}
				}
			}
		}
		_self._master = true;
		_self.onFinishCompiling = objArg.onFinishCompiling;
		_registerPlugins.call(_self);
	});
}
/**--module--
...
var _registerPlugins = function(){
	...
	while(i--){
		...
		if(!BrowserEnvironment && typeof allPlugins[key] === "undefined"){
			plugin = require('./../plugins/' + key + '/' + key).Plugin;
		...
		if(typeof allPlugins[key] !== "undefined"){
			...
			if(BrowserEnvironment && !plugin.browserFriendly) Utilities.warning(this,this.language.warning("The following plugin will not work for the browser: ") + this._plugins[i]);
			if(typeof plugin.onTemplateCreate !== "undefined") plugin.onTemplateCreate(this,function(){
				num--;
			...
			require(['./../plugins/' + key + '/' + key + '.js'],function(plugin){
				plugin = plugin.Plugin;
				if(typeof plugin.pluginName === "undefined") throw "The following plugin needs a plugin name: "+ plugin.toString();
				if(BrowserEnvironment && !plugin.browserFriendly) Utilities.warning(_self,_self.language.warning("The following plugin will not work for the browser: ") + _self._plugins[i]);
				if(typeof plugin.onTemplateCreate !== "undefined") plugin.onTemplateCreate(_self,function(){
					num--;
