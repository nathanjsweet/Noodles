...
var _registerPlugins = function(){
	...
	while(i--){
		...
		if(typeof allPlugins[key] !== "undefined"){
			...
			if(BrowserEnvironment && !plugin.browserFriendly) Utilities.warning(this,this.language.warning("The following plugin will not work for the browser: "), _self.language.others(plugin.pluginName));
			...
			require(['./../plugins/' + key + '/' + key + '.js'],
			//Callback on require
			function(plugin){
				plugin = plugin.Plugin;
				if(typeof plugin.pluginName === "undefined") throw "The following plugin needs a plugin name: "+ plugin.toString();
				if(BrowserEnvironment && !plugin.browserFriendly) Utilities.warning(_self,_self.language.warning("The following plugin will not work for the browser: "), _self.language.others(plugin.pluginName) );
				if(typeof plugin.onTemplateCreate !== "undefined") plugin.onTemplateCreate(_self,
					//callback on template require
					function(){
						num--;
